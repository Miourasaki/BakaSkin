...
@router.get("/parse/{uuid}/skin")
async def parse_skin(uuid: str):
    UserData = db("users").find_one({"_id": uuid_encode(uuid)})
    if not UserData:
        return HTTPException(status_code=404)

    processed_image = process_image(UserData["textures"]["SKIN"]["url"])
